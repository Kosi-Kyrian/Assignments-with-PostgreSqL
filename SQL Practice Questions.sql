/*1. Create a new database called store_db, then run the scripts in the 
	products.sql file.*/ 
...
/*8. Find all products in the Home category OR with a stock quantity greater 
	than 100.*/
SELECT * FROM products 
WHERE category = 'Home' 
...
/*13. List all products sorted by rating in descending order, then by price in 
	ascending order when ratings are equal.*/
SELECT * FROM products 
...
/*26. Find all products in either the Books or Stationery category with a price
	  less than $25.*/
SELECT * FROM products 
WHERE category = 'Books' AND price < 25 
OR category = 'Stationery' AND price < 25;
...
SELECT id, CONCAT(name, '-', category) FROM products;
...
SELECT name, price, ROUND(price-(price*0.15)) FROM products;

--34. Display the day of the week (name) when each product was released.
SELECT name, release_date, TO_CHAR(release_date, 'day') FROM products;
...
/*36. Calculate how many days each product has been in inventory since its 
	release date.*/
SELECT name, release_date, CURRENT_DATE - release_date
FROM products;
...
SELECT COUNT(is_available) FROM products 
WHERE is_available = 'true';

/*41. Find the average price, minimum price, maximum price, and total number of
	products in a single result.*/
SELECT ROUND(AVG(price),2), MIN(price), MAX(price), COUNT(name) FROM products;